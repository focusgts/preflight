...
import type { PreFlightFinding, RuleDefinition } from './types';
...
export function isPomFile(path: string): boolean {
  return /pom\.xml$/i.test(path) || /\.pom$/i.test(path);
}

export function isIndexDefinition(path: string, content: string): boolean {
  const pathHint = /oak[_:]index|_oak_index|index-definition/i.test(path);
  const contentHint =
    /oak:QueryIndexDefinition|type\s*=\s*["'](?:lucene|property)["']/i.test(
      content
    );
...
const sonarQubeRules: RuleDefinition[] = [
  {
    ...
    category: 'sonarqube',
    description:
      'HttpClient or URLConnection created without connection/socket timeout configuration. Cloud Manager requires all HTTP connections to have explicit timeouts.',
    remediation:
      'Set connectTimeout, socketTimeout, and connectionRequestTimeout via RequestConfig.custom(). Recommended: 10s connect, 30s socket.',
    check(content, filePath) {
      const findings: PreFlightFinding[] = [];
      if (!isJavaFile(filePath)) return findings;

      const httpClientPattern =
        /new\s+HttpClient\s*\(|HttpClients\.create(?:Default)?\s*\(|HttpClientBuilder\.create\s*\(/g;
      const urlConnectionPattern = /\.openConnection\s*\(/g;
      const timeoutPattern =
        /\.setConnectTimeout\s*\(|\.setSocketTimeout\s*\(|\.setConnectionRequestTimeout\s*\(|ConnectTimeoutException|RequestConfig\.custom\s*\(\s*\)[\s\S]{0,500}\.setConnectTimeout|\.setReadTimeout\s*\(|\.setConnectTimeout\s*\(/;
      ...
      if (hasHttpClient && !hasTimeout) {
        findings.push({
          ...
          line: findLineNumber(
            content,
            /new\s+HttpClient\s*\(|HttpClients\.create|HttpClientBuilder/
          ),
          message:
            'HttpClient created without connection/socket timeout configuration. Cloud Manager requires all HTTP connections to have explicit timeouts.',
          remediation:
            'Set connectTimeout, socketTimeout, and connectionRequestTimeout via RequestConfig.custom(). Recommended: 10s connect, 30s socket.',
        });
      }
      ...
      if (hasUrlConnection && !hasTimeout) {
        findings.push({
          ...
          message:
            'URLConnection opened without setConnectTimeout/setReadTimeout. Cloud Manager requires explicit timeouts on all connections.',
          remediation:
            'Call setConnectTimeout() and setReadTimeout() immediately after openConnection(). Recommended: 10000ms connect, 30000ms read.',
        });
      }
...

  {
    ...
    category: 'sonarqube',
    description:
      'ResourceResolver opened but not closed in finally block or try-with-resources. This causes resource leaks that crash AEMaaCS pods.',
    remediation:
      'Wrap ResourceResolver in try-with-resources: try (ResourceResolver resolver = factory.getServiceResourceResolver(params)) { ... }',
    check(content, filePath) {
      const findings: PreFlightFinding[] = [];
      if (!isJavaFile(filePath)) return findings;

      const resolverCreation =
        /(?:getServiceResourceResolver|getResourceResolver|getAdministrativeResourceResolver)\s*\(/g;
      ...
      if (resolverCreation.test(content)) {
        ...
        if (!hasTryWith && !hasFinallyClose) {
          findings.push({
            ...
            line: findLineNumber(
              content,
              /(?:getServiceResourceResolver|getResourceResolver)\s*\(/
            ),
            message:
              'ResourceResolver opened but not closed in finally block or try-with-resources. This causes resource leaks that crash AEMaaCS pods.',
            remediation:
...

  {
    ...
    category: 'sonarqube',
    description:
      "Servlet registered by path instead of resourceType. Path-bound servlets bypass AEM's security model and are blocked by Cloud Manager.",
    remediation:
      'Register servlets by sling.servlet.resourceTypes and sling.servlet.selectors/extensions instead of sling.servlet.paths.',
    check(content, filePath) {
      const findings: PreFlightFinding[] = [];
      if (!isJavaFile(filePath)) return findings;

      const pathBinding =
        /sling\.servlet\.paths\s*=|@SlingServlet\s*\([^)]*paths\s*=|property\s*=\s*"sling\.servlet\.paths/;
...

  {
    ...
    category: 'sonarqube',
    description:
      'ResourceResolver stored as instance field in an OSGi component. Resolvers must be request-scoped or opened/closed within a single method.',
    remediation:
      'Open ResourceResolver in a try-with-resources block inside each method that needs it. Do not cache resolvers as instance fields.',
    check(content, filePath) {
      const findings: PreFlightFinding[] = [];
      if (!isJavaFile(filePath)) return findings;

      const fieldResolver =
        /(?:private|protected)\s+ResourceResolver\s+\w+\s*;/;
      ...
      if (openCount > 0 && openCount > closeCount + 1) {
        findings.push({
          ...
          remediation:
            'Ensure every getServiceResourceResolver() / getResourceResolver() call has a matching close() in a finally block or uses try-with-resources.',
        });
      }
...

  {
    ...
    category: 'sonarqube',
    description:
      'Direct /libs content reference found. /libs is immutable in Cloud Service and may change without notice.',
    remediation:
      'Overlay the component into /apps and reference the overlay. Use sling:resourceSuperType for component inheritance instead of direct /libs paths.',
    check(content, filePath) {
      const findings: PreFlightFinding[] = [];

      if (!isJavaFile(filePath) && !isContentFile(filePath)) return findings;

      const matchLines = findAllLineNumbers(
        content,
        /["'`]\/libs\/[^"'`]+["'`]/
      );
...

  {
    ...
    category: 'sonarqube',
    description:
      'AMS-specific run mode detected. Cloud Service uses environment-specific run modes without the config. prefix pattern.',
    remediation:
      'Replace AMS run modes with Cloud Service equivalents. Use config.author/config.publish for tier targeting and env-specific variables via Cloud Manager environment variables.',
    check(content, filePath) {
      const findings: PreFlightFinding[] = [];
      ...
      if (matchLine !== undefined) {
        findings.push({
          ...
          line: matchLine,
          message:
            'AMS-specific run mode detected. Cloud Service uses environment-specific run modes (dev, stage, prod) without the config. prefix pattern.',
          remediation:
            'Replace AMS run modes with Cloud Service equivalents. Use config.author/config.publish for tier targeting and env-specific variables via Cloud Manager environment variables.',
        });
      }
      ...
      if (
        ...
        amsRunModes.some(
          (m) => filePath.includes(`/${m}/`) || filePath.endsWith(`/${m}`)
        )
      ) {
        findings.push({
          ruleId: 'CQRules:AMSRT',
          ...
          filePath,
          message:
            'OSGi config file in AMS-specific run mode folder. Cloud Service does not support AMS run mode folder conventions.',
          remediation:
            'Restructure to config.author/, config.publish/, or config.author.dev/ (Cloud Service naming). Use Cloud Manager environment variables for environment-specific values.',
        });
      }
...
const oakpalRules: RuleDefinition[] = [
  {
    ...
    category: 'oakpal',
    description:
      'Custom Oak index uses type="property". Cloud Manager requires custom indexes to use type="lucene" for AEMaaCS.',
    remediation:
      'Change index type from "property" to "lucene". Property indexes are managed by Adobe in Cloud Service and cannot be customized.',
    check(content, filePath) {
      const findings: PreFlightFinding[] = [];
      if (!isIndexDefinition(filePath, content)) return findings;

      const hasPropertyType =
        /type\s*=\s*["']property["']/i.test(content) ||
        /jcr:primaryType\s*=\s*["']oak:QueryIndexDefinition["'][\s\S]*?type\s*=\s*["']property["']/i.test(
          content
        );
...

  {
    ...
    category: 'oakpal',
    description:
      'Lucene index definition missing async flag. Synchronous lucene indexes are not supported in AEMaaCS.',
    remediation:
      'Add async="[async, nrt]" property to the index definition node. This ensures the index is updated asynchronously.',
    check(content, filePath) {
      const findings: PreFlightFinding[] = [];
      if (!isIndexDefinition(filePath, content)) return findings;
...

  {
    ...
    category: 'oakpal',
    description:
      'Full-text index definition missing Tika configuration. Without explicit Tika config, binary extraction may fail or produce unexpected results in Cloud Service.',
    remediation:
      'Add a tika child node with appropriate mime type configuration. Reference Adobe documentation for damAssetLucene Tika config structure.',
    check(content, filePath) {
      const findings: PreFlightFinding[] = [];
      if (!isIndexDefinition(filePath, content)) return findings;

      const isFullText =
        /evaluatePathRestrictions\s*=\s*["']\{Boolean\}true["']/i.test(
          content
        ) ||
...

  {
    ...
    category: 'oakpal',
    description:
      'damAssetLucene index does not follow required naming convention. Cloud Manager enforces the pattern: damAssetLucene-{version}-custom-{version}.',
    remediation:
      'Rename the index to follow the pattern: damAssetLucene-8-custom-1 (where 8 is the base OOTB version and 1 is your custom version).',
    check(content, filePath) {
      const findings: PreFlightFinding[] = [];
      if (!isIndexDefinition(filePath, content)) return findings;

      const isDamIndex =
        /damAssetLucene/i.test(filePath) || /damAssetLucene/i.test(content);
      if (!isDamIndex) return findings;

      const validNaming = /damAssetLucene-\d+-custom-\d+/i;
      const hasValidName =
        validNaming.test(filePath) || validNaming.test(content);
...

  {
    ...
    category: 'oakpal',
    description:
      'Index compatVersion is below 2. Cloud Service requires compatVersion >= 2 for Lucene indexes.',
    remediation:
      'Add or update: compatVersion="{Long}2" on the index definition node.',
    check(content, filePath) {
      const findings: PreFlightFinding[] = [];
      if (!isIndexDefinition(filePath, content)) return findings;
      ...
      const compatMatch = content.match(
        /compatVersion\s*=\s*["']\{(?:Long|long)\}(\d+)["']/i
      );
...

  {
    ...
    category: 'oakpal',
    description:
      'Index path restrictions reference volatile or invalid locations in Cloud Service.',
    remediation:
      'Remove or update path restrictions to reference /content, /apps, or /oak:index paths only.',
    check(content, filePath) {
      const findings: PreFlightFinding[] = [];
      if (!isIndexDefinition(filePath, content)) return findings;
...
const javaCompatRules: RuleDefinition[] = [
  {
    ...
    description:
      'Uses internal sun.* packages which are removed/encapsulated in Java 17+. AEMaaCS runs Java 11/17/21.',
    remediation:
      'Replace sun.misc.Unsafe with VarHandle, sun.reflect.* with java.lang.invoke.*, sun.security.* with java.security.*. See JEP 403 for migration guidance.',
    check(content, filePath) {
      const findings: PreFlightFinding[] = [];
      if (!isJavaFile(filePath)) return findings;

      const sunImportPattern = /import\s+sun\.\w+/g;
      const sunUsagePattern =
        /sun\.(misc|reflect|security|nio|net|awt|font|util)\.\w+/;
...

  {
    ...
    category: 'java-compat',
    description:
      'Uses the old javax.* namespace. AEMaaCS SDK 2024.x+ uses the jakarta namespace.',
    remediation:
      'Replace javax.* imports with jakarta.* equivalents. Update corresponding Maven dependencies.',
    check(content, filePath) {
      const findings: PreFlightFinding[] = [];
      if (!isJavaFile(filePath)) return findings;
...

  {
    ...
    category: 'java-compat',
    description:
      'Uses reflection which may fail under Java 17+ strong encapsulation (JEP 403). Cloud Service JVM does not permit --add-opens overrides.',
    remediation:
      'Replace reflective access with public APIs, MethodHandles, or VarHandles. If accessing AEM internals, use the official Sling/OSGi APIs instead.',
    check(content, filePath) {
      const findings: PreFlightFinding[] = [];
      if (!isJavaFile(filePath)) return findings;
      ...
      const reflectionPatterns = [
        ...
        {
          pattern:
            /\.getDeclaredField\s*\([^)]+\)[\s\S]{0,100}\.setAccessible/g,
        ...
        {
          pattern:
            /Class\.forName\s*\([^)]*\)[\s\S]{0,200}\.getDeclaredConstructor/g,
...

  {
    ...
    category: 'java-compat',
    description:
      'Native library loading detected. Native libraries compiled for x86-64 will fail on ARM-based Cloud Service VMs.',
    remediation:
      'Replace native library dependencies with pure Java alternatives. If unavoidable, provide both x86-64 and aarch64 binaries.',
    check(content, filePath) {
      const findings: PreFlightFinding[] = [];
      ...
      for (const { pattern, desc } of nativePatterns) {
        ...
        if (pattern.test(content)) {
          findings.push({
            ...
            remediation:
              'Replace native library dependencies with pure Java alternatives. If unavoidable, provide both x86-64 and aarch64 binaries and use architecture detection at load time.',
          });
        }
      }
      ...
      if (isPomFile(filePath)) {
        ...
        if (jniPattern.test(content)) {
          findings.push({
            ...
            message:
              'POM references JNI/JNA/native dependency. Native dependencies require ARM compatibility for AEMaaCS.',
            remediation:
              'Verify the native dependency provides aarch64 (ARM) binaries. Consider a pure Java alternative if ARM support is missing.',
          });
        }
      }
