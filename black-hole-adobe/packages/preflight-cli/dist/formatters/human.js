...
function formatHuman(report) {
    ...
    if (report.findings.length === 0) {
        lines.push('');
        lines.push(colorize('green', `Pre-Flight Results: 0 findings -- all clear`));
        lines.push('');
        return lines.join('\n');
    }
    lines.push('');
    ...
    for (const severity of SEVERITY_ORDER) {
        ...
        for (const finding of findings) {
            ...
            lines.push(`    ${colorize('gray', '->')} ${finding.remediation}`);
        }
        lines.push('');
    }
    // Summary line
    ...
    if (report.summary.blockers > 0) {
        summaryParts.push(colorize('red', `${report.summary.blockers} blocker${report.summary.blockers !== 1 ? 's' : ''}`));
    }
    if (report.summary.criticals > 0) {
        summaryParts.push(colorize('yellow', `${report.summary.criticals} critical`));
    }
    if (report.summary.majors > 0) {
        summaryParts.push(colorize('blue', `${report.summary.majors} major`));
    }
    if (report.summary.minors > 0) {
        summaryParts.push(colorize('gray', `${report.summary.minors} minor`));
    }
    const hasBlockersOrCriticals = report.summary.blockers > 0 || report.summary.criticals > 0;
    const marker = hasBlockersOrCriticals
        ? colorize('red', 'x')
        : colorize('yellow', '!');
    lines.push(`${marker} ${summaryParts.join(', ')} -- ${hasBlockersOrCriticals ? 'pipeline will fail' : 'review recommended'}`);
    lines.push('');
    return lines.join('\n');
}
function severityLabel(severity, count) {
